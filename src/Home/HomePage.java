...
public class HomePage extends javax.swing.JFrame {
    ...
    private void initComponents() {
        ...
        Home_tab.add(jScrollPane3, new org.netbeans.lib.awtextra.AbsoluteConstraints(30, 270, 790, 140));
        ...
        Home_tab.add(jPanel2, new org.netbeans.lib.awtextra.AbsoluteConstraints(190, 180, 470, 50));
    ...
    private void nBudgetButtonMouseClicked(java.awt.event.MouseEvent evt) {
    ...
    private void nBudgetButtonMouseEntered(java.awt.event.MouseEvent evt) {
        nBudgetButton.setBackground(new java.awt.Color(0, 68, 68));
    }

    private void nBudgetButtonMouseExited(java.awt.event.MouseEvent evt) {
        nBudgetButton.setBackground(new java.awt.Color(0, 102, 102));
    }
    ...
    private void addBudgetExpenseCategoryButtonActionPerformed(java.awt.event.ActionEvent evt) {
        String expenseCategory = BudgetExpenseCategoryTextField.getText().trim();
        addExpenseCategoryFunction(expenseCategory);
    }

    private void addBudgetActionPerformed(java.awt.event.ActionEvent evt) {
    ...
    if (!expenseCategory.isEmpty() && !expenseCategory.equals("--select--")) {
        if (!budgetAmountText.isEmpty()) {
            try {
                ...
                String checkQuery = "SELECT * FROM budget WHERE user_id = ? AND expense_category = ?";
                ...
                int rowsAffected = 0;
                if (rs.next()) {
                    String updateQuery = "UPDATE budget SET amount = ? WHERE user_id = ? AND expense_category = ?";
                    PreparedStatement updateStmt = DatabaseManager.getConnection().prepareStatement(updateQuery);
                    updateStmt.setDouble(1, amount);
                    updateStmt.setInt(2, UserSession.userId);
                    updateStmt.setString(3, expenseCategory);
                    ...
                    PreparedStatement insertStmt = DatabaseManager.getConnection().prepareStatement(insertQuery);
                    insertStmt.setInt(1, UserSession.userId);
                    insertStmt.setString(2, expenseCategory);
                ...
                // Reset form and update UI components
                if (rowsAffected > 0) {
    ...
    private void addTargetAmountButtonActionPerformed(java.awt.event.ActionEvent evt) {
    ...
    private void removeBudgetButtonActionPerformed(java.awt.event.ActionEvent evt) {
    ...
    private void addExpenseCategoryActionPerformed(java.awt.event.ActionEvent evt) {
        String expenseCategory = expenseCategoryTextField.getText().trim();
        addExpenseCategoryFunction(expenseCategory);
    }

    private void ExpenseResetButtonActionPerformed(java.awt.event.ActionEvent evt) {
    ...
    private void addIncomeSourceActionPerformed(java.awt.event.ActionEvent evt) {
    ...
    private void incomeAmountActionPerformed(java.awt.event.ActionEvent evt) {
    }
    ...
    private void expenseAccountNameActionPerformed(java.awt.event.ActionEvent evt) {
    }
    ...
    private void createAccountActionPerformed(java.awt.event.ActionEvent evt) {
        String accountName = jTextField1.getText().trim(); 
    ...
    private void incomeSourceComboBoxActionPerformed(java.awt.event.ActionEvent evt) {
    }

    private void incomeAccountNameActionPerformed(java.awt.event.ActionEvent evt) {
    }
                    ...
                    if (accountResult.next()) {
                        ...
                        if (rowsUpdated > 0) {
                            ...
                            if (rowsInserted > 0) {
                                ...
                                expenseRemark.setText("");
                                expenseAccountName.setSelectedIndex(0);
                                checkBudgetAfterExpense(expenseCategory,amount);
                                // populate_expense_table();
                                // populateTable();
    ...
    private void addIncomeButtonActionPerformed(java.awt.event.ActionEvent evt) {
        ...
        java.sql.Date incomedate = new java.sql.Date(incomeDate.getDate().getTime());
        int accountId = 0;
    ...
    private void resetIncomeButtonActionPerformed(java.awt.event.ActionEvent evt) {
        incomeDate.setDate(null);
        incomeSourceComboBox.setSelectedIndex(0);
        incomeAmount.setText("");
        incomeAccountName.setSelectedIndex(0);
    ...
    DefaultTableModel model = new DefaultTableModel(
        new String[] {"Expense Category", "Budget Amount", "Actual Expense", "Remaining", "Status"}, 0);
    ...
    try {
        ...
        String query = "SELECT b.expense_category, b.amount as budget_amount, " +
                      "COALESCE(SUM(e.amount), 0) as actual_expense " +
                      "FROM budget b " +
                      "LEFT JOIN expense e ON b.expense_category = e.expense_category " +
                      "AND MONTH(e.expense_date) = ? AND YEAR(e.expense_date) = ? " +
                      "AND b.user_id = e.user_id " +
                      "WHERE b.user_id = ? " +
                      "GROUP BY b.expense_category, b.amount " +
                      "ORDER BY b.expense_category";
        ...
        while (rs.next()) {
            ...
            double remaining = budgetAmount - actualExpense;
            String status = remaining < 0 ? "OVER BUDGET" : "OK";
            
            // Add a row to the table model
            model.addRow(new Object[] { expenseCategory, budgetAmount, actualExpense, remaining, status });
        }
        
        budgetTable.setModel(model);
        
        // Add conditional formatting
        addBudgetTableFormatting();
        
        // Add progress bars
        addBudgetProgressBars();
...
private void addBudgetTableFormatting() {
    budgetTable.setDefaultRenderer(Object.class, new javax.swing.table.DefaultTableCellRenderer() {
        ...
        public java.awt.Component getTableCellRendererComponent(javax.swing.JTable table, 
            ...
            // Check if the table has at least 5 columns and there are rows
            if (table.getColumnCount() >= 5 && row >= 0 && row < table.getRowCount()) {
                // Get the status value (column 4)
                Object statusObj = table.getModel().getValueAt(row, 4);
                if (statusObj != null) {
                    String status = statusObj.toString();
                    
                    // Set background color based on status
                    if ("OVER BUDGET".equals(status)) {
...
private void addBudgetProgressBars() {
    // Check if the progress column already exists
    if (budgetTable.getColumnCount() <= 5) {
        // Add a new column for progress bars
        DefaultTableModel model = (DefaultTableModel) budgetTable.getModel();
        model.addColumn("Usage");
        
        // Create a custom renderer for progress bars
        budgetTable.getColumnModel().getColumn(5).setCellRenderer(new javax.swing.table.TableCellRenderer() {
            ...
            public java.awt.Component getTableCellRendererComponent(javax.swing.JTable table, 
                ...
                // Get budget and actual expense values
                if (row >= 0 && row < table.getRowCount() && 
                    table.getModel().getValueAt(row, 1) != null && 
                    table.getModel().getValueAt(row, 2) != null) {
                    
                    double budget = Double.parseDouble(table.getModel().getValueAt(row, 1).toString());
                    double actual = Double.parseDouble(table.getModel().getValueAt(row, 2).toString());
                    
                    // Calculate percentage used
                    int percentUsed = budget > 0 ? (int)Math.min(100, (actual / budget) * 100) : 0;
                    
                    // Set progress bar properties
                    progressBar.setValue(percentUsed);
                    progressBar.setString(percentUsed + "%");
                    
                    // Set color based on usage
                    if (percentUsed >= 90) {
                        progressBar.setForeground(new java.awt.Color(255, 0, 0)); // Red
                    } else if (percentUsed >= 75) {
                        progressBar.setForeground(new java.awt.Color(255, 165, 0)); // Orange
                    } else {
                        progressBar.setForeground(new java.awt.Color(0, 128, 0)); // Green
                    }
                }
...
private void checkBudgetAlerts() {
    try {
        ...
        // Query to find categories where expenses exceed budget
        String query = "SELECT b.expense_category, b.amount as budget_amount, " +
                      "SUM(e.amount) as actual_expense " +
                      "FROM budget b " +
                      "JOIN expense e ON b.expense_category = e.expense_category " +
                      "AND MONTH(e.expense_date) = ? AND YEAR(e.expense_date) = ? " +
                      "AND b.user_id = e.user_id " +
                      "WHERE b.user_id = ? " +
                      "GROUP BY b.expense_category, b.amount " +
        ...
        // Build alert message if any categories are over budget
        StringBuilder alertMessage = new StringBuilder("Budget Alert:\n");
        ...
        while (rs.next()) {
            ...
            double budgetAmount = rs.getDouble("budget_amount");
            double actualExpense = rs.getDouble("actual_expense");
            double overAmount = actualExpense - budgetAmount;
            
            alertMessage.append("- ").append(category).append(": Over by ")
                       .append(String.format("%.2f", overAmount)).append("\n");
        }
        
        // Show alert if any categories are over budget
        if (hasOverBudget) {
        ...
        System.err.println("Error checking budget alerts: " + ex.getMessage());
    }
}

private void checkBudgetAfterExpense(String category, double amount) {
    try {
        // Get current month and year
        java.util.Calendar cal = java.util.Calendar.getInstance();
        int currentMonth = cal.get(java.util.Calendar.MONTH) + 1;
        ...
        // Query to get budget and total expenses for this category
        String query = "SELECT b.amount as budget_amount, " +
                      "(SELECT COALESCE(SUM(e.amount), 0) FROM expense e " +
                      "WHERE e.expense_category = ? " +
                      "AND MONTH(e.expense_date) = ? AND YEAR(e.expense_date) = ? " +
                      "AND e.user_id = ?) as total_expense " +
                      "FROM budget b " +
                      "WHERE b.expense_category = ? AND b.user_id = ?";
        ...
        pstmt.setInt(4, UserSession.userId);
        pstmt.setString(5, category);
        pstmt.setInt(6, UserSession.userId);
        ...
        if (rs.next()) {
            ...
            if (totalExpense > budgetAmount) {
                double overAmount = totalExpense - budgetAmount;
                
                JOptionPane.showMessageDialog(null, 
                    "Budget Alert: Your expenses for category '" + category + 
                    "' have exceeded the budget by " + String.format("%.2f", overAmount),
                    "Budget Warning", JOptionPane.WARNING_MESSAGE);
            }
        }
        ...
        System.err.println("Error checking budget after expense: " + ex.getMessage());
    }
}

private void addBudgetSummaryToHome() {
    try {
        // Get current month and year
        java.util.Calendar cal = java.util.Calendar.getInstance();
        int currentMonth = cal.get(java.util.Calendar.MONTH) + 1;
        ...
        // Query to get overall budget status
        String query = "SELECT " +
                      ...
                      "AND YEAR(e.expense_date) = ?), 0) as total_expense " +
        ...
        if (rs.next()) {
            ...
            // You'll need to add a label to your Home tab to display this information
            // For example:
            // budgetSummaryLabel.setText("Budget: " + totalBudget + " | Spent: " + 
            //    totalExpense + " | Remaining: " + remaining);
        }
        
        rs.close();
        pstmt.close();
    } catch (SQLException ex) {
        System.err.println("Error adding budget summary to home: " + ex.getMessage());
    }
}
